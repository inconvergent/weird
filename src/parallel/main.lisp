...
(defun init (&key (cores 4) (name "custom-kernel"))
  (setf lparallel:*kernel* (lparallel:make-kernel cores :name name)))

(defun end () (lparallel:end-kernel :wait t))
...
(defun info ()
  (let ((name (lparallel:kernel-name))
    ...
    (format t "~&kernel name = ~a~%worker threads count = ~d~%kernel context = ~a~%kernel bindings = ~a~%"
      name count context bindings)))

(defun create-channel () (lparallel:make-channel))
