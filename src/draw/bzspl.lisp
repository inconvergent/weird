    ...
    (veq:vlabels
      ((-to-res (pts) (veq:f$_ (map 'list #'second pts)))
       ...
       (-adaptive (l r (:va 2 lv rv) res)
         (let ((m (-midsample l r)))
           (declare (veq:ff m))
           (veq:f2let ((mv (-x-to-pt vpts ns m)))
      ...
      (-to-res
        (let ((m (-midsample 0f0 1f0))
              (res (make-adjustable-vector :type 'list)))
          (declare (veq:ff m) (vector res))
          (veq:f2let ((mv (-x-to-pt vpts ns m)))
            (-adaptive 0f0 m (-x-to-pt vpts ns 0f0) mv res)
            (-adaptive m 1f0 mv (-x-to-pt vpts ns 1f0) res))
          res)))))
...
(defmacro -set-mean (vpts opts a b c)
  `(veq:fvprogn (veq:2$vset (,vpts ,c)
                  (veq:f2iscale (veq:f2+ (veq:f2$ ,opts ,a ,b)) 2f0))))
